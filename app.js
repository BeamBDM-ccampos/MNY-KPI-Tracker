...
function renderDisplayBtg() {
  ...
  const label = MODE_CONFIG[state.mode].secondaryLabel;

  if (!trackerData.displayBtg.length) {
    section.innerHTML = emptySection(`${label} BTG`, `Load tracker data to see ${label} BTG.`);
    return;
  }

  section.innerHTML = `
    <div class="report-section">
      <div class="section-title">${escapeHtml(label)} BTG</div>
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
...
async function sendBdmRepReports() {
  ...
  try {
    ...
    setBdmEmailStatus(
      `Email sent to ${email} with ${result.pdfCount || 0} ${MODE_CONFIG[state.mode].secondaryLabel}/POD PDF report(s).`
    );
...
function updateModeLabels() {
  ...
  const trackerReportTitle = document.getElementById("trackerReportTitle");
  const printSelectionTitle = document.getElementById("printSelectionTitle");
  const accountSelectionTitle = document.getElementById("accountSelectionTitle");
  const emailText = document.getElementById("bdmBulkEmailText");
  ...
  if (trackerReportTitle) {
    trackerReportTitle.textContent = trackerName;
  }

  if (printSelectionTitle) {
    printSelectionTitle.textContent = `${trackerName} — Account Selections`;
  }

  if (accountSelectionTitle) {
    accountSelectionTitle.textContent = "Account Selection";
  }

  if (emailText) {
    emailText.textContent =
      `This sends one email with a separate POD BTG and ${config.secondaryLabel} BTG PDF for each Sales Person under the selected BDM.`;
  }

  if (pageTitle) {
    pageTitle.textContent = trackerName;
  }
}
